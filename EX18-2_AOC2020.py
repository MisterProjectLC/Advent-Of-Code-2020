...
sumRegex = re.compile("(\d+) (\+) (\d+)")
multiRegex = re.compile("(\d+) (\*) (\d+)")
...
for line in lines:
    ...
    while (firstMatch != None):
        ...
        result = getResult(sumRegex, text)
        result = getResult(multiRegex, result)
        
        line = parenRegex.sub(str(result), line, 1)
        firstMatch = parenRegex.search(line)

    # get entire result
    line = getResult(sumRegex, line)
    line = getResult(multiRegex, line)
    total += int(line)
